...
public class UserDao {
    ...
    public void save(String account,String pwd,String username,String sex,String phonenumber){
            String sql = "INSERT INTO user (account,pwd,username,sex,avatar,phonenumber,registerdate,grade,ismerchant) " +
    ...
    public void save(String account, String pwd, String username, String sex) {
        String sql = "INSERT INTO user (account,pwd,username,sex,avatar,registerdate,grade,ismerchant) " +
                "VALUES(?,?,?,?,'imgs/tx0.jpg',NOW(),60.0,'否')";
    ...
    public List<User> findMerchant(){
        List<User> list = new ArrayList<User>();
        String sql = "SELECT * FROM user where ismerchant like '是' ";
        ResultSet rs = DBUtil.select(sql);
        try {
            while (rs.next()) {//指标往下移动一行
                int id2 = rs.getInt(1);
    ...
    public void modifyIsm(String ism, int id) {
        String sql = "UPDATE user SET ismerchant =? WHERE userid=?";
